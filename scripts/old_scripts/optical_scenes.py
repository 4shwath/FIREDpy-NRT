...
from sentinelsat import SentinelAPI, geojson_to_wkt, LTATriggered, make_path_filter
...
def download_product(api, product_id, output_directory, path_filter, num_retries=5, retry_wait=30):
    """
    ...
    for i in range(num_retries):
        try:
            ...
            api.download(product_id, directory_path = output_directory, nodefilter=path_filter)
...
def get_sentinel(event_id,
    ...
    if products:
        ...
        #print(f'These are the products:')
        ...
        if config.download_scenes:
            ...
            path_filter1 = make_path_filter("*_B0[2348]_10m.jp2")
            path_filter2 = make_path_filter("*_B8A_20m.jp2")
            path_filter3 = make_path_filter("*_B12_20m.jp2")
            api.download_all(products, directory_path = output_directory, nodefilter=path_filter1)
            api.download_all(products, directory_path = output_directory, nodefilter=path_filter2)
            api.download_all(products, directory_path = output_directory, nodefilter=path_filter3)
            ...
            #HERE
            #for p_id in products:
            #    path_filter = make_path_filter("*_B(?:0[2348]|8a|12)_(?:10|20)m.jp2")
            #    download_product(api, p_id, output_directory, path_filter)
...
def main(event_id, satellite):
    ...
    print(f"Searching for {satellite} images for event ID {fid} between {start_date} and {end_date}")
    ...
    if satellite == "sentinel":
        ...
        # For pre-fire, range should be (start_date - delta, start_date)
        # For post-fire, range should be (end_date, end_date + delta)
